...
>
> 范围：`--keep-sandbox`（裸 = failed/errored，`=always` 全收）、留存注册表 `.niceeval/sandboxes/`、`niceeval sandbox list` / `stop`、run 收尾 Kept sandboxes 输出与启动残留提醒、`result.json` 新增 `sandbox` 字段（provider / sandboxId / kept，沙箱型 attempt 无条件落盘）。不做：`Sandbox` 接口的 pause/detach 方法、自定义 provider 留存、留存沙箱的 resume/重评、`sandbox enter` 交互式子命令（打印 enter 命令即可）。
