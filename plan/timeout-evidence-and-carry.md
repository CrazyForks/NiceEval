...
- [ ] **B. 指纹与携带**(src/runner/fingerprint.ts + planCarry)
  - [ ] B1. fingerprint 哈希输入移除 timeoutMs(src/runner/fingerprint.ts:47 附近)
  - [ ] B2. planCarry 追加资格判据:`durationMs ≤ 当前 resolved timeoutMs`(未设=无穷);逐 attempt 判定,与 [[carry-must-be-per-attempt-not-whole-eval-key]] 的既有纪律一致
