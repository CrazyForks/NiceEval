...
| coding agent CLI（claude-code / codex / bub 等改文件的任务） | https://niceeval.com/docs/zh/tutorials/sandbox-agent（要配 `sandbox`，默认 `dockerSandbox()`，先跟用户确认后端） |
...
| 其它自研 agent loop、LangGraph、OpenAI Agents SDK、已部署 agent | https://niceeval.com/docs/zh/tutorials/connect-your-agent 起步，手写 send 的完整教程在 https://niceeval.com/docs/zh/tutorials/write-send |
| 纯函数、没有独立服务的场景 | 先读 https://niceeval.com/docs/zh/tutorials/connect-your-agent 里"为什么不直调"那段，跟用户确认这确实是他们要的边缘用法，再继续 |
