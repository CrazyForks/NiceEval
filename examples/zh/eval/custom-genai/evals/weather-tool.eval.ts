...
// 天气问题要走 get_weather,不许模型自己编数字 —— agents/custom-genai.ts 把 server.ts
// 返回的 toolCalls 拆成 action.called/action.result 对,负断言(notCalledTool)因此可信。
export default defineEval({
  description: "深圳天气走 get_weather,参数、顺序、负断言全部可断言",

  async test(t) {
    const turn = await t.send("深圳今天天气怎么样?");
    turn.expectOk();

    await t.group("调用 get_weather 且城市正确", () => {
      t.calledTool("get_weather", { input: { city: "深圳" } });
