...
async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === "POST" && req.url === "/api/chat") {
    // Tier 3 侵入点:instructions / tools 是请求体可选字段,不传时走应用默认行为。
    const body = await readJson(req) as { messages?: unknown[]; model?: string; instructions?: string; tools?: string[] };
    const signal = abortSignalFor(req);
    const overrides = { instructions: body.instructions, tools: body.tools };
    const result = await streamChat(body.messages ?? [], body.model, signal, overrides);
    pipeUIMessageStreamToResponse({
      response: res,
      stream: toUIMessageStream({ stream: result.stream, tools: buildTools(body.tools) }),
      headers: corsHeaders(),
    });
    return;
  }
