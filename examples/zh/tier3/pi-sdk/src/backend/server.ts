...
async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === "POST" && url === "/api/chat") {
    const body = (await readJson(req)) as { message?: unknown; sessionId?: unknown; systemPrompt?: unknown };
    if (typeof body.message !== "string" || body.message.trim().length === 0) {
      throw new Error("body.message must be a non-empty string.");
    }
    // Tier 3 侵入点:systemPrompt 是请求体可选字段,不传时走应用默认 prompt。
    if (body.systemPrompt !== undefined && typeof body.systemPrompt !== "string") {
      throw new Error("body.systemPrompt must be a string when present.");
    }
    const sessionId = typeof body.sessionId === "string" && body.sessionId ? body.sessionId : randomUUID();
    await streamChat(req, res, body.message, sessionId, body.systemPrompt);
    return;
  }
