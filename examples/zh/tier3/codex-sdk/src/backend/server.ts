...
// Tier 3 侵入点:请求体可选字段 sandboxMode 的合法取值(@openai/codex-sdk 的 SandboxMode)。
...
async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === "POST" && url === "/api/chat") {
    const { message, threadId, sandboxMode } = parseChatRequest(await readJson(req));
    ...
    try {
      const events = await runTurnStreamed(message, threadId, abort.signal, sandboxMode);
...
function parseChatRequest(value: unknown): { message: string; threadId?: string; sandboxMode?: SandboxMode } {
  if (typeof value !== "object" || value === null) throw new Error("JSON body is required.");
  const record = value as Record<string, unknown>;
  if (typeof record.message !== "string" || record.message.trim().length === 0) {
    throw new Error("message must be a non-empty string.");
  }
  if (record.sandboxMode !== undefined && !SANDBOX_MODES.includes(record.sandboxMode as SandboxMode)) {
    throw new Error(`sandboxMode must be one of: ${SANDBOX_MODES.join(", ")}.`);
  }
  return {
