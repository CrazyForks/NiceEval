# langgraph 示例：niceeval Tier 3 接入（侵入改造 + experiment params）
...
- `src/backend/server.py`:`/api/chat` 请求体多一个可选字段 `systemPrompt`(类型校验),
  ...
  不再读模块全局。
- `agents/langgraph.ts`:experiment 的 `params.systemPrompt` 经 `ctx.params` 随请求体透传。
...
agents/langgraph.ts                     →  ctx.params.systemPrompt 塞进请求体
...
单配置基线 `pnpm exec niceeval exp langgraph` 仍然可用(不带 params,应用走默认行为)。
