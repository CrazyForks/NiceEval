...
# Tier 3 侵入点:checkpointer 提到模块级共享——system prompt 变体各自编译一张图,但共用
# 同一个 saver,同一个 thread_id 的会话记忆跨变体延续(interrupt/resume 的检查点也在这里)。
_SAVER = InMemorySaver()


def build_agent(system_prompt: str | None = None):
    """Tier 3 侵入点:system prompt 提升为可选参数,不传时行为与改造前完全一致。"""
    llm = ChatOpenAI(
