...
# Tier 3 侵入点:system prompt 提升为请求体可选字段 systemPrompt。同一个 prompt 变体的图
# 只编译一次,之后缓存复用;所有变体共享同一个 checkpointer(agent.py 的 _SAVER),同一个
# thread_id 的会话记忆与 interrupt/resume 检查点跨变体延续。不带 systemPrompt 的请求
# 走 "" 这格缓存,行为与改造前完全一致。
_agents: dict[str, object] = {"": build_agent()}
_agents_lock = threading.Lock()


def _agent_for(system_prompt: str | None):
    key = system_prompt or ""
    with _agents_lock:
        agent = _agents.get(key)
        if agent is None:
            agent = _agents[key] = build_agent(system_prompt)
    return agent
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            system_prompt = body.get("systemPrompt") or None
            if system_prompt is not None and not isinstance(system_prompt, str):
                raise ValueError("body.systemPrompt must be a string when present.")
        except (ValueError, json.JSONDecodeError) as error:
            self._send_json(400, {"error": str(error)})
            return
        self._stream_chat(message, session_id, system_prompt)
    ...
    def _stream_chat(self, message: str, session_id: str | None, system_prompt: str | None = None) -> None:
        ...
        try:
            for event in _run_turn(message, thread_id, _agent_for(system_prompt)):
                if not send(event):
                    return
...
def _run_turn(message: str, thread_id: str, agent):
    ...
    yield from _drive_graph(inputs, config, agent)


def _drive_graph(graph_input, config: dict, agent):
    """喂一次 `agent.stream(graph_input, config, ...)`,直到这一轮自然结束或者
    ...
    `agent.get_graph().nodes` 能看到同样的名字。`HumanInTheLoopMiddleware` 命中
    ...
    for stream_mode, payload in agent.stream(graph_input, config, stream_mode=["messages", "updates"]):
        ...
        if "__interrupt__" in payload:
            ...
            yield from _drive_graph(Command(resume={"decisions": decisions}), config, agent)
            return
