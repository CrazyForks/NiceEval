...
// Tier 3 侵入点:system prompt 提升为请求级可选参数,不传时保持默认行为不变。
// feature A/B 见 experiments/compare-prompts/。
export function runTurn(message: string, resumeSessionId: string | undefined, systemPrompt?: string): Query {
  return query({
    prompt: message,
    options: {
