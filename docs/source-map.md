...
- **`--json` 机器面的题目级 `eval` 事件与单一 renderer 已在源码落地,plan 台账尚未收口**:`src/runner/feedback/{agent,ci}.ts` 已删除,`src/runner/feedback/json.ts` 是唯一的机器面 renderer(`profile.ts` 的 `OutputProfile` 只剩 `"human"`/`"json"` 两态),docs/feature/experiments/cli.md「runs 与首过即停怎样展示」描述的题目级 `eval` 事件(early exit 时 `attempts`/`planned`/`unstarted`/`reason:"early_exit"`,跑满时 `attempts`/`passed`;`verdict` 取代表 attempt 自身判定、`locator` 按同一份代表 attempt 选取规则)由 `evalConclusionRows`(`src/runner/feedback/eval-conclusions.ts`)派生、`writeEvalConclusions` 写出,字段与选取规则同文档一致(`assembleInvocationCompletion()` 派生 `InvocationCompletion.earlyExitUnstarted`,命名已是 `InvocationCompletion`)。这批改动已随 exp 两形态实现提交;`plan/exp-json-machine-form.md` 的 TODO 树与 `plan/docs-code-gap-closure.md` 节点 D.4(`pnpm e2e --repo cli` 真实 stdout 验收)的勾选收口待 planner 对账——前者的 agent/ci 双 profile 前提已被单一 json renderer 取代,收口时核对 plan 描述与现状是否还对得上。
...
- **`Usage` 落盘形状与 `src/o11y/types.ts` 不一致**:`docs/feature/results/architecture.md#usage` 的目标 `Usage` 是 `inputTokens?`/`outputTokens?`(均可选,协议不提供就省略)、`cacheReadTokens?`、`cacheCreationTokens?`、`reasoningTokens?`、`requests?`,不含 `costUSD`(成本是消费端估算,不落在 `Usage` 里)。当前 `src/o11y/types.ts` 的 `Usage` 是 `inputTokens: number`/`outputTokens: number` 必填、字段名是 `cacheWriteTokens` 不是 `cacheCreationTokens`、没有 `reasoningTokens`、多出 `costUSD?: number`。这套旧形状波及 `src/context/session.ts`、`src/runner/{attempt,types}.ts`、`src/show/{render,compose}.ts`、`src/agents/{langgraph,sdk-streams,ai-sdk}.ts`、`src/runner/reporters/braintrust.ts`、`src/o11y/{types,cost}.ts`、`src/o11y/parsers/{bub,openclaw}.ts`、`src/scoring/scoped.ts`、`src/report/{index,model/{metrics,types}}.ts`、`src/report/components/{shared,fixtures}.ts`、`src/report/components/attempt-detail/{compute,faces,index,AttemptSummary,AttemptUsage}.tsx?`、`src/report/components/entity-lists/{compute,index,AttemptList,ExperimentList,faces}.tsx?`、`src/report/components/summaries/{compute,index}.tsx` 共 30 余个非测试文件。修复见实施计划 `plan/show-scope-slice-json.md` 节点 A1(「`Usage` 形状对齐 `src/types.ts`」)。
- **`DeltaTable` 多条件对照尚未落地,当前实现仍是两侧配对**:`docs/feature/reports/library/metric-views.md#deltatable` 的目标契约是 `conditions`(任意条数,不限两侧)× 固定 `score`/`tokens`/`cost` 三个指标逐 eval 一行的对照矩阵;当前 `src/report/model/types.ts` 的 `DeltaData`/`DeltaPair` 与 `src/report/components/metric-views/compute.ts` 的 `deltaTableData` 仍是两侧 `pairs` 配对 + 可配置 `MetricColumn` 的 `cells`,`pairsByFlag()` 只在 `by` 为 `"experiment"` 时机械导出全部 A/B 两两对。修复见实施计划 `plan/show-scope-slice-json.md` 节点 C1。
- **`StabilityMatrix` 完全缺席**:`docs/feature/reports/library/metric-views.md#stabilitymatrix` 定义的历史全执行稳定性矩阵(行 eval、列 `by` 维度取值、格是跨快照身份去重后的判定计数,不设可比性门槛,服务「这道题历史上稳不稳」而非现刻水位)在 `src/report/components/metric-views/` 没有对应的 `StabilityMatrixData`/`stabilityMatrixData`/组件文件,`show --stats` 也没有对应零配置装配。修复见实施计划 `plan/show-scope-slice-json.md` 节点 C1b。
- **`UsageTable` 组装口径未落地,当前 `AttemptUsage` 只是落盘 `Usage` 的原样透传**:`docs/feature/reports/library/attempt-detail.md#usagetable-组装口径单源` 的目标 `UsageTableData` 是 `{ locator, experimentId, evalId, attempt, verdict, turns?, toolCalls?, usage?, uncachedInputTokens?, estimatedCostUSD? }`——5 个身份字段(`locator`/`experimentId`/`evalId`/`attempt`/`verdict`)+ 事件流派生的 `turns`/`toolCalls` + 两个输入都在场时派生的 `uncachedInputTokens`,是 attempt 首页 `usage:` 行、`--usage` 表、对照矩阵用量列、`--execution` turn 头行共用的单一组装口径。当前 `src/report/components/attempt-detail/compute.ts` 的 `attemptUsageData(evidence)` 只返回 `{ usage, costUSD }`,没有身份字段、没有 `turns`/`toolCalls`、没有 `uncachedInputTokens` 派生;text 面呈现格式也未对齐目标行文案(见 `docs/feature/reports/library/attempt-detail.md` 示例 `usage: 6 turns · 21 tool calls · …`)。这不是简单改名——`UsageTable` 是切片组件化(补身份/行为字段 + 派生量)的结果,不是 `AttemptUsage` 换个导出名。修复见实施计划 `plan/show-scope-slice-json.md` 节点 C2。
- **`show` CLI 面的三轴模型(范围 × 切片 × 形态)尚未实现,当前实现比「未实现」更危险的是两处静默/暗行为**:`docs/feature/reports/show.md` 定稿的范围(`--exp` 可重复、`@locator`)× 切片(`--source`/`--execution`/`--usage`/`--stats`/`--diff`/裸报告)× 形态(人读文本 / `--json`)三轴组合模型,当前 `src/cli.ts` 的 `FLAG_OPTIONS` 里 `exp: { type: "string" }` 没有 `multiple: true`(`node:util` `parseArgs` 对不带 `multiple` 的 `string` 类型 flag 重复传入时静默只保留最后一次出现的值,不报错、不警告)——比契约要求的「支持重复 `--exp` 收窄多个 experiment」单纯「未实现」更危险:用户传两次 `--exp` 会以为选中了两个 experiment,实际只有最后一个生效。`--stats`/`--usage`/`--json`/`--grep`/`--expand` 五个切片/形态 flag 均未出现在 `FLAG_OPTIONS`(`src/cli.ts` 全文搜索 `--stats`/`--usage`(show 专属)/`--grep`/`--expand` 均无命中;`--json` 布尔项存在但按其 JSDoc 与 `resolveOutputForm` 调用点只服务 `exp` 命令,不服务 `show`)。证据切面(`--source`/`--execution`/`--timing`/`--diff`)当前限定单 attempt,不支持范围内多 attempt 分节输出。此外 `src/show/index.ts:333-348` 还有一条 docs 未记载的 legacy 分支:`flags.report === undefined && flags.page === undefined && patterns.length > 0 && matchedEvalIds.length === 1` 时整个绕过报告槽 / `resolveReportTree` 管线,直接用 `evalDetailText` 渲染一份独立的单 eval 详情文本——这条「前缀恰好命中一个 eval」的特判不属于三轴模型里的任何一根轴,`docs/feature/reports/show.md` 未描述它,收口三轴模型时需要显式裁决是保留(补进文档)还是被范围语义取代。修复见实施计划 `plan/show-scope-slice-json.md`(尤其节点 B1/B2)。
