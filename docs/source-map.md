...
- **Snapshot diagnostics 尚未落地**:目标契约要求 `snap.finish({ diagnostics })` 在封口 `snapshot.json` 时持久化实验域诊断,并由公开 `SnapshotDiagnostics` 组件在三张内建 scope-input page 按真实 Snapshot 来源呈现。当前 `src/results/writer.ts` 的 `finish()` 只补 `completedAt`,`src/report/components/site-components/` 与 `src/report/built-in/standard.tsx` 也还没有该组件及接线;实现时须同批补纯数据投影、text/web 两面、公开导出与内建报告结构测试。
...
- **组深度视图(得分点 = 组)尚未实现**:[计分粒度 · 折叠树](feature/experiments/score-points.md#折叠树判定面分数面质量分)与[报告读取面](feature/experiments/score-points.md#报告读取面show-与-view-怎么读)要求 `MetricMatrix` 能读「行 = eval × 组」的格子(计分制读组内挣分、通过制读组质量分)。当前 `MetricMatrix`(`src/report/components/metric-views/compute.ts` 的 `metricMatrixData`)的 `cell: Metric` 模型是「一个 Metric 均匀应用到全部格子」,`Metric.value(attempt)` 不接收「当前格子对应哪个 `groupPath`」这个上下文,`DimensionInput`(`src/report/model/types.ts`)也没有「按 `t.group` 运行时分组路径分组」这一种内置维度(`evalGroup` 是 eval id 的路径前缀,与 `t.group` 的 groupPath 是两个不同概念)。这不是缺一行代码,而是需要新设计:要么给 `MetricMatrix` 引入依赖行维度值的动态 Metric 形态,要么是专门的新组件——需要回到设计裁决,不在本节点内自行发明。docs 的字面契约先按现状保留,留作后续设计。
- **agent/ci 尚无 eval 级聚合行**:当前实现名 `RunCompletion.earlyExitUnstarted` 已从反馈状态派生(`earlyExitSkipped` 减去 fail-fast 份额,见 `src/cli.ts` `assembleRunCompletion()`),目标契约名为 `InvocationCompletion`;docs/feature/experiments/cli.md「runs 与首过即停怎样展示」里的逐 eval 结论行(`NICEEVAL eval … attempts= planned= unstarted= reason=early_exit` / 跑满时的 `rate=`)在 `src/runner/feedback/{agent,ci}.ts` 还没有对应 renderer,当前只有 Invocation 级 handoff 汇总。
