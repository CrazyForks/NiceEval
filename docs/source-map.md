...
- **`show` CLI 面的三轴模型(范围 × 切片 × 形态):范围解析与既有证据切面已按 `plan/show-scope-slice-json.md` 节点 B1/B2 收口,`--stats`/`--usage`/`--json`/`--grep`/`--expand` 仍待 C/D 节点接线**:`src/cli.ts` 的 `FLAG_OPTIONS.exp` 已带 `multiple: true`,`src/show/index.ts` 的 `flags.experiment` 是 `string[]`;0/1 个 `--exp` 沿用前缀收窄语义(可能匹配多个 experiment),2 个以上按 `assertExperimentSelectors` 校验——每个必须恰好解析到一个 experiment,命中多个按用法错误列出候选(`cli.show.expAmbiguous`),`@<locator>` 与重复 `--exp` 互斥(`cli.show.locatorExpConflict`)。证据切面(`--source`/`--execution`/`--timing`/`--diff`)已接受任意范围:`renderEvidenceSections`(`src/show/index.ts`)对排序后(experimentId、evalId、attempt 序)的每个 attempt 复用同一套 `evalSourceText`/`executionText`/`timingText`/`diffText` 渲染件,`@<locator>` 单元素范围与多 attempt 范围走同一份实现,不再有「locator 专属」分支。原 `src/show/index.ts:333-348` 的 legacy 单 eval 详情分支(`evalDetailText`)已删除——eval 前缀命中单个 eval 时并入默认榜单(报告槽),与裸 show / 单个 `--exp` 同规则;`evalDetailText`/`attemptIndexLine`/`pickDetailAttempt`/`attemptsOfEval`/`attemptHeader`/`displayAttemptNumber` 一并从 `src/show/render.ts` 删除(不再有生产调用点)。尚未落地的是缺省切片选择表第二行(`--exp` ≥ 2 的对照矩阵):`src/show/index.ts` 的 `renderCompareSlice` 是节点 C1(`DeltaTable`)的诚实占位——2 个以上 `--exp` 且没有 `--report` 接管时抛 `cli.show.compareNotWired`,不静默渲染;`--stats`/`--usage`/`--json`/`--grep`/`--expand` 五个切片/形态 flag 仍未出现在 `FLAG_OPTIONS`(`src/cli.ts` 全文搜索均无命中;`--json` 布尔项按其 JSDoc 与 `resolveOutputForm` 调用点仍只服务 `exp` 命令)。修复见实施计划 `plan/show-scope-slice-json.md` 节点 C/D。
