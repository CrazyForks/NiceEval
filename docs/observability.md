...
```json
{
  ...
  "evals": [
    { "id": "weather/brooklyn", "verdict": "passed", "outcome": "passed",
      "assertions": [
...
`verdict` 是评分折叠结果;`outcome` 是报告分类。`summary.failed` 与 `summary.errored` 是互斥计数:前者表示断言/评分不通过,后者表示环境、超时、adapter 或 agent runtime 这类执行错误。JUnit reporter 也按这个口径输出 `<failure>` 与 `<error>`。
