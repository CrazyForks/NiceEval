...
- `INIT.zh.md`：安装前向导。它工作的阶段包还不在 `node_modules`，从官网 / GitHub raw 读取；安装完成后一切以随包 `INDEX.md` 的路由为准。
...
- **不做手写任务表**（按任务分组、一行路由多页组合的人工索引）。页面自述已单源存在于 frontmatter，手写行是它的复述，复述必然漂移，而守护只能保证路径存在、保证不了措辞同步；跨页组合的路由，agent 扫一遍带自述的树即可自行完成。若效果评估（如 onboarding eval）表明 agent 路由质量不足，再以有证据的策展补一张小表，不默认维护。
