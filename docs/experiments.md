...
export default defineExperiment({
  ...
  params?: Record<string, unknown>;          // 任意 KV 参数,透传到 ctx.params / t.params(见下)
...
### model / reasoningEffort 与 params:agent 留空,实验决定
...
- **`params`** —— 任意 KV 的参数,**两处可见**:agent 的 `send`(`ctx.params`)、eval 的 `test`(`t.params`)。用来开关联网、注入某个 skill、或让某条 eval 只在某个参数下断言。
...
export default defineExperiment({
  ...
  params: { webResearch: true, skill: "memory-v2" }, // → ctx.params / t.params
  runs: 3,
});
```

参数驱动的环境差异(比如按 `params.skill` 往沙箱注入一个 skill 文件)写在 eval 的 `test(t)` 里:`if (t.params.skill) await t.sandbox.writeFiles({ ".agent/skill.md": loadSkill(t.params.skill) })`——普通代码,不需要框架钩子。
...
- **文件 = 单一配置**(一个 agent × 一个 model 是最干净的情形),文件名按 `<agent>-<model>[-<feature>]` 命名。同组钉住对比轴之外的一切(如同一 model),差异才干净归因到那一个轴(agent / 记忆机制 / params)。
...
| `webResearch` / `agentOptions` | `params` | **合并** | 一个通用参数袋取代散落的开关,经 `ctx.params` / `t.params` 透传 |
...
调度项覆盖优先级(高 → 低):**CLI flag → experiment → config → 内置默认**。agent、model、params 属于 experiment,不由 CLI 覆盖。
