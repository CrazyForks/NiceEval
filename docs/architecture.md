...
四段职责是**单向数据流**:发现产出一批 `Eval`,运行器逐个对 Agent `send` 得到 `RunResult`,评分器把 `RunResult` 折叠成 `Assertion[]`,判决把断言折叠成一个 `Verdict`,再派生报告用的互斥 `Outcome`;报告器消费结果分类 + 工件。没有反向耦合 —— 评分器不知道 agent 的 transport 是进程内、HTTP 还是沙箱 CLI,它只看 `RunResult`。
...
9. **判决。** `verdict.ts` 把断言 + 执行错误 + 跳过原因折叠成评分语义的 `verdict`,再把执行错误拆成报告语义的 `outcome=errored`。
...
12. **退出码。** 有 `outcome=failed` / `outcome=errored`(或 `--strict` 下有 `scored`)→ 非零退出;报告里 failure 与 error 分开,供 CI 判红和诊断。
