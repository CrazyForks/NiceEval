...
fasteval view [结果目录|summary.json]  # 起本地 web 查看器,读 .fasteval/ 历史运行出图
```
...
```sh
fasteval view                         # 起本地 web,读 .fasteval/
...
默认运行会写 `.fasteval/<timestamp>/summary.json` 与 `results.jsonl`;`view` 直接读这些结构化工件。当前查看器先提供 Next.js evals 风格的密集榜单:按 experiment 聚合、可排序、可搜索、可展开看单个 eval attempt 的断言/错误/用量。
