...
- **连 AI / 接 agent:** [Agents 与 Adapters(定位与导航)](adapters/README.md)、[Origin 应用接入手册(examples/zh/origin 五个应用的接入工单)](origin-integration.md)、[OTel Mixin(已接 OTel 时免写转换器:otelEvents + 方言表)](adapters/otel-mixin.md)、[Adapter 契约(逐 API 适配义务)](adapters/contract.md)、[Adapter 写法(分档递进)](adapters/authoring.md)、[采集设计(三条外部路线对比)](adapters/collection.md)、[接入目标矩阵(12 个被测对象调研)](adapters/targets.md)、[Coding Agent Skills / Plugins DX](adapters/coding-agent-skills-plugins.md)、[Sandbox](sandbox.md)。
- **设计提案(未实现):** [Capabilities by Construction(能力由实现证明)](capabilities-by-construction.md)、[Multi-Agent(多 agent eval 的三种场景)](multi-agent.md)、[E2E CI(全链路 CI 测试方案)](e2e-ci.md)。
