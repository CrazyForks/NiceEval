...
- **Agent** 是"一条连到 AI 的连接"的抽象,按名字选(`--agent <name>`);**Adapter** 是它的具体实现,**由你编写**(claude-code / codex 等内置)。fasteval **不定义任何 agent 协议** —— 连你自己的服务也是你写一个 agent,URL 是它的内部配置,没有 `--url`。
...
npx fasteval             # 发现并运行所有 eval
npx fasteval weather     # 位置参数 = id 前缀,只跑 id 以 weather 开头的 eval
npx fasteval --watch     # 改文件即重跑
```
