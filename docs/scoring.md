...
无执行错误且 verdict=failed                 → failed   (断言/评分不通过)
有执行错误                                  → errored  (环境、超时、adapter、agent runtime)
verdict=scored / skipped                    → scored / skipped
```

因此 `summary.failed` 只数真·不通过,`summary.errored` 单独数执行/环境问题。旧的 `verdict === "failed"` 仍可能代表执行错误,做报告和 CI 集成时优先用 `outcome`。
