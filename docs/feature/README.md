...
- `use-case/`：CLI 的每个位置参数与 flag 配一组用例文档，一篇讲一个用户用例的全流程——用户遇到什么问题、带上它之后从命令到结束反馈的完整路径、边界与何时改用别的模式。契约单源仍在 `cli.md` 与对应功能页，用例篇只做叙事串联，不复制契约定义；目录自带 `README.md` 按 flag 分组列出各篇。
