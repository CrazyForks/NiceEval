...
2. 某个 attempt 的 send 撞限流。兜底分类器从错误文本认出限流关键字 → `rate_limit`,进入退避重试;activity 行如实显示,期望形态:
...
5. 限流持续到 4 次尝试全部耗尽时,该 attempt 照常 `errored`,message 带重试摘要(`… · retries exhausted (4 attempts, rate_limit)`),fail-fast 看到的已是这个干净得多的最终信号。`errored` 不进指纹缓存——限流窗口过去后**重跑同一条命令即是续跑**,只补跑失败的那几个 attempt。

## 边界

- **重试参数固定,不可调。** 封顶 4 次、基数 5 秒是[非目标](../README.md#非目标)里定死的值;总退避上界约 35 秒,限流持续更久说明并发本身超出配额,把 `--max-concurrency` 降下来才是对因下药,重试只兜抖动不兜超卖。
