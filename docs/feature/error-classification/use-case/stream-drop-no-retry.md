...
attempt `errored`,message 是响应中途的流中断或连接重置——看起来明明是基建抖动,框架却没有重试。这不是遗漏:message 里**没有** `retries exhausted` 后缀,说明这个错误被判为不可重试、从未进入重试。这篇讲怎么读这个结果、为什么设计如此、下一步做什么。
...
- [README](../README.md) —— 可重试/不可重试的判据与理由。
