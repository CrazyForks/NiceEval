...
实体列表用于从汇总下钻到事实，不允许自由配置列。固定列不等于所有渲染面使用相同排版：web 面可以用表格支持人工比较，text 面可以用紧凑列表支持终端阅读，但两面必须消费同一份 `.data()` 结果（计算契约见 [Library 总览](../library.md#数据计算与缓存边界)）。
...
- [指标组件](metric-views.md) —— 从实体切换到指标视角。
