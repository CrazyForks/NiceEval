...
text 面先输出与 web 同列口径的 experiment 比较表（列集合随主读数规则，与 web 面一致），再按 experiment 输出 Eval / Attempt 明细表。Eval 是父行，不是 Attempt 行上的重复字段；Attempt 用 `├─` / `└─` 子行显示一对多关系。明细列固定为状态、Eval / Attempt、结果、耗时、成本；窄终端复用标准 text table renderer 折行或从右侧隐藏低优先级列，并明确报告隐藏列数：
...
计分制 Scope 的同一张表把主列换成总分，Eval / Attempt 明细行各自附挣分：
