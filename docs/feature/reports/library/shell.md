...
- **web 面恒含 `Powered by NiceEval` 品牌行。** hero 之下是指向 niceeval 官网的一行品牌色小字。它是外壳自带的品牌行：不占 `footer` 的语义位、没有关闭配置、不改变任何数据；`footer` 文案单独渲染在页面底部，省略 `footer` 时不渲染页脚。text 面与 `niceeval/report/react` 嵌入组件都不带品牌行——品牌跟着官方 web 外壳走，不跟着组件走。
