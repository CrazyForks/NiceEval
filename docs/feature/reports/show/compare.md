...
榜单回答「每个条件整体多好」，对照矩阵回答「每道题在条件间怎么变」；两者消费同一套 Scope 选择，只是投影不同。报告库的 [`DeltaTable`](../library/metric-views.md) 服务自定义报告的 web/text 面；对照矩阵是宿主证据投影，零配置、不经报告树。

## 条件与配对

- 每个 `--exp` 是一个对照条件，按出现顺序排列，**第一个是基准**。每个 `--exp` 必须恰好解析到一个 experiment；前缀匹配到多个时按用法错误退出并列出全部候选 id。
- 配对身份是 **eval id**：同一 eval id 在各条件下的结果进同一行。eval id 前缀位置参数照常收窄行集。
- 每格是该 experiment × eval 的折叠：verdict 用与榜单同一套题目级判定口径；tokens 与成本是该题全部 attempt 的合计，attempt 数大于 1 时在格内标 `×N`。
- 某条件没有该题的结果时，格显示 `—`（覆盖缺口，语义同榜单的[占位行](default-report.md)），该题不计入该条件的汇总分母；行首的 eval 仍然出现，缺席不是隐藏行的理由。
...
- 翻转标记 `⇄` 只出现在各条件判定不一致的行；全部一致的行不加噪声。
- `Δ` 列是该条件对基准的**原始差值**（对照值 − 基准值），方向好坏由指标的 `better` 语义判断，矩阵不替读者下结论——与榜单散点位移摘要同一条规则。任一侧缺数据的行 `Δ` 为 `—`，缺失不当 0。
- 条件超过两个时，每个非基准条件各带自己的 `Δ` 列组，全部对第一个 `--exp` 求差；输出变宽是允许的，宽内容交给终端横向滚动，不为省列宽合并语义。
- `汇总` 行是各条件自身覆盖面的描述，分母是该条件有结果的 eval 数；它允许回答「这批结果各自总体怎样」，但不拿两个不同分母直接做归因。
- `共同题对基准` 只在基准与该条件都存在的 eval 交集上计算 paired delta：先在同一题上配对，再分别聚合通过率、tokens 与成本。覆盖缺口另由头部的共同 / 仅某条件计数表达，不能把候选独有的一道题混进改动效果，也不能把缺失当 0。上例共同 7 题中基准 4/7、候选 6/7，所以是 `+28.6pt`；候选完整覆盖的 `7/8` 只留在汇总行。
- 数值列跟随 Scope 主读数映射（[题型构成与主读数](../library/metrics.md#题型构成与主读数)）：通过制显示 verdict，计分制在 verdict 位显示挣分（如 `3 pt`；计分制没有满分声明），混型按题型分段。
- 混型范围的汇总与 paired delta 同样分段：通过制子集报通过率，计分制子集报总分，tokens / 成本仍可在全部共同题上合计。不能把两种题型压成一个“综合分”，也不能拿通过制交集的 eval 数去当计分制分母。
...
niceeval show --exp A --exp B --json         # 对照矩阵的结构化输出（形状见 --json 分篇）
...
- 与 `--report` 互斥（缺省切片被报告树替换时，对照矩阵不再适用；对比组件在报告里用 `DeltaTable`）。
