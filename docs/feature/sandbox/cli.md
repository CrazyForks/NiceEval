...
`sandbox` 命令组不读 `niceeval.config.ts`、不发现 eval,只操作留存注册表(`.niceeval/sandboxes/` 下的逐条目文件,见 [Architecture · 留存注册表](architecture.md#留存keep与注册表))。`stop` 的语义与 `Sandbox.stop()` 一致:销毁,没有暂停态。正因为事后命令不执行用户项目代码,`defineSandbox` 自定义 provider 不支持 `--keep-sandbox`;组合使用会在创建沙箱前报错。
