...
- **结构化 artifact。** 事件流、transcript、文件 diff、断言结果都落盘成机器可读格式,可回放、可二次分析。
...
- **核心** 发现 eval、收集断言、计算判定、调度并发、做缓存、写报告与 artifact。它对每个被测对象一视同仁。
...
- [Agents 与 Adapters](feature/adapters/README.md)、[Sandbox](feature/sandbox/README.md) —— 连接与沙箱契约。
