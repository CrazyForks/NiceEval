...
// results.latest() 口径(裸跑填充 defaultReport,en / zh-CN 双语渲染)、跨快照去重让
// Runs/Traces 不被 --resume 复印件灌票。viewData 只携带证据室数据(快照明细 + skipped
// + 壳元信息),不再有 overview / table / overall 统计产物。
...
describe("loadViewScan · 报告槽是 latest 口径,不再跨历史合并", () => {
  it("同一实验两次快照:报告槽只反映最新快照;partial-coverage 警告由报告槽内的 RunOverview 呈现", async () => {
    const root = await makeRoot();
    // 周一全量:q1 失败、q2 通过。
    await writeSnapshot(root, "exp_a", "2026-07-01T08-00-00-000Z", { experimentId: "exp/a", agent: "bub", startedAt: "2026-07-01T08:00:00.000Z" }, [
      res("q1", "failed"),
    ...
    const { viewData, reportHtml } = await loadViewScan(root);
    // 旧的 aggregateRows 会把两次快照揉在一起(q1 一败一过);latest 口径只看周二快照:
    // 全过(100%),报告槽的深链指向周二那次快照的 attempt。
    expect(reportHtml.en).toContain("100%");
    expect(reportHtml.en).toContain("#/attempt/exp_a/2026-07-02T08-00-00-000Z/");
    ...
    // 挑选警告由报告槽内的 RunOverview 呈现(壳不设第二条通道),不静默。
    expect(reportHtml.en).toContain('data-kind="partial-coverage"');
    expect(reportHtml.en).toContain("1 of 2");
...
describe("loadViewScan · 跨快照去重(--resume 携带的复印件只算一次)", () => {
  it("同一 attempt 存在于两份落盘:只保留最新快照里的那份,attemptRef 落在最新落盘,artifactBase 沿用原快照 artifact", async () => {
    const root = await makeRoot();
    ...
    expect(q1.attemptRef).toEqual({ snapshot: "exp_a/2026-07-02T08-00-00-000Z", attempt: "q1/a0" }); // 证据身份跟着最新落盘走
...
describe("loadViewScan · 新布局落盘直接可读(写入面 / 读取面同一契约)", () => {
  it("快照与 attemptRef 都从落盘产物算出", async () => {
    const root = await makeRoot();
    await writeSnapshot(
      root,
      "compare_bub",
    ...
    const { viewData, reportHtml } = await loadViewScan(root);
    // 报告槽(defaultReport 的 web 面):榜单行 + 官方通过率格子(1 过 1 败 = 50%)。
    ...
    expect(snapshot.results.every((r) => r.attemptRef?.snapshot === snapshot.run)).toBe(true);
    ...
    expect(q1.artifactBase).toBe(`${q1.attemptRef!.snapshot}/${q1.attemptRef!.attempt}`);
  });
});
