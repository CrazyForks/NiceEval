...
describe("loadViewScan · skipped 三种原因进 viewData", () => {
  it("incompatible-version / malformed / incomplete 都进 skippedRuns;niceeval 落盘拼 npx 命令,第三方如实报名字不拼", async () => {
    ...
    // incomplete:有 attempt artifact、没有 summary(run 中途 crash)。
...
describe("loadViewScan · 跨快照去重(--resume 携带的复印件只算一次)", () => {
  it("同一 attempt 存在于两份落盘:只保留最新 run 里的那份,attemptRef 落在最新落盘, artifactBase 沿用原 run artifact", async () => {
    const root = await makeRoot();
    const original = res({
      id: "q1",
      agent: "bub",
    ...
    // 携带条目: artifactsDir 清空, artifactBase 指向原 run 的 artifact 目录(runner 的 carriedResults 形状)。
    ...
    expect(q1.artifactBase).toBe("2026-07-01T08-00-00-000Z/artifacts/q1/bub/default/a0"); // artifact 仍指原 run
...
describe("loadViewScan · createRunWriter 写出的落盘直接可读(writer/reader 同一契约)", () => {
  it("writer roundtrip:默认报告、快照与 attemptRef 都从写入面产物算出", async () => {
    ...
    // 写入面拆出的 artifact,读取面拼回相对 view 根的 artifactBase。
