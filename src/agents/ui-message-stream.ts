...
export function uiMessageStreamAgent(options: UiMessageStreamAgentOptions): Agent {
  ...
  // 这里只多存两份本协议特有的状态:chat id(useChat 协议要求请求带,每条会话线固定一个)
  // 和 lastReported(HITL 续跑时跨轮去重要用),都锚在会话线的 state 上,随线丢弃。
  const history = clientHistory<UIMessageLike>();
  const chatIds = new WeakMap<object, string>();
  const lastReportedBySession = new Map<string, ReportedState>();

  return defineAgent({
    name: options.name ?? "ui-message-stream",
    session: history, // 件即能力:conversation 由构造派生
    capabilities: { toolObservability: true, ...options.capabilities },
    ...
    async send(input: TurnInput, ctx: AgentContext) {
      ...
      const priorMessages = history.get(ctx);
      let id = chatIds.get(ctx.session.state);
      if (!id) {
        id = `uims-${randomUUID()}`;
        chatIds.set(ctx.session.state, id);
        if (!ctx.session.id) ctx.session.id = id; // 镜像到旧字段,t.sessionId / 报告可见
      }
