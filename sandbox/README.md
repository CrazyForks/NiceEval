...
已发布的 `v0.6.1` 早于这条规范化：其中 Claude Code 基线的默认 npm prefix 仍是只读的
`/usr`。下一组公共模板发布并更新源码常量前，在 `v0.6.1` 运行时安装全局工具应使用
...
三份模板都要通过构建脚本里的 npm prefix / PATH / 写权限自检后才能发布；发布完成并真机验证后
再统一 bump `src/sandbox/e2b-agent-template.ts` 的 release 常量，不能让常量提前指向不存在的制品。
