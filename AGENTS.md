...
`docs/` 与 `docs-site/` 的正文只写当前定稿的契约与行为，声明式表述：

- 不写相对旧稿的差分句——「去掉 X 后的类型」「删除了 Y」「不再是 Z」这类措辞不落正文；契约要不依赖读者知道旧稿、独立成立。
- 不留待定问题，不写「要不要 / 再议 / 再考虑」——分歧在定稿前裁决，落进文档的只有决定和理由。
- 决策记录（如 `docs/reports.md` 的落选表、裁决记录）可以叙述被否方案与理由，这是例外且要标注日期；但它旁边的契约描述本身仍须是声明式的。
