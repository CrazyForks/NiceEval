...
- **You don't have to guess the answer from free text.** `input.responses` carries `{ requestId, optionId }` per request (or `{ requestId, text }` for free-text answers); an `optionId` that hits one of the request's options has already been validated on the eval side, so a typo throws instead of silently reaching the application. See [Inputs for the different answers](/explanation/adapter) for what each answer shape looks like once it reaches the adapter.
...
Approval and rejection are two branches of the same gate — coverage is only complete when you write an eval for each: what should happen after approval happens, and what should happen after rejection does not. See [Drive](/explanation/drive) for the full API-by-API usage.
...
The paused execution state (for example, a partially read SSE stream) lives on the session line, in `ctx.session`: call `ctx.session.hold(state)` on the paused turn and `ctx.session.take()` on the response turn to retrieve it — `take()` clears it once read. See [Write Send](/tutorials/write-send) step 5 for the full skeleton of how this fits together with streaming.

Like the rest of [NiceEval](https://niceeval.com/)'s capabilities, HITL is not declared with a boolean flag: **doing it is having it**. If `send` has returned `"waiting"` and emitted `input.requested`, `t.respond` works; if it has not, the eval fails explicitly at the first `parked()` or `requireInputRequest()`, instead of silently passing. See [Adapter](/explanation/adapter) for how capabilities follow from construction.
...
- [Drive](/explanation/drive) — The full eval-side usage: `t.parked()`, `t.requireInputRequest()`, `t.respond()` / `t.respondAll()`.
