...
Judge calls are scoring functions like any other assertion, so they follow the same mechanics described in [Assert · Gate vs soft severity](/explanation/assert) — but their **default** is different from most value matchers:
