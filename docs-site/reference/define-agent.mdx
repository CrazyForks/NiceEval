...
The `shared` object from `fasteval/adapter` provides utilities that are common across all sandbox agent adapters, ensuring that workspace preparation, diff collection, validation, and observability injection work consistently regardless of which agent CLI you're wrapping.
...
The following coding agent adapters are exported by fasteval and can be referenced from experiment files:
...
```shell
npx fasteval exp claude-code-local evals/fixtures/button --sandbox docker
npx fasteval exp codex-local evals/fixtures/button --sandbox docker
```
