---
title: "用 Coding Agent 编写 Eval 并迭代程序"
sidebarTitle: "Coding Agent 反馈闭环"
description: "让 Coding Agent 读取随包文档，用 Bash 运行实验、查看结果、定位失败并持续迭代，最后通过全量重跑确认结果。"
---

Coding Agent 可以根据需求编写 NiceEval 配置与 Eval，也可以根据运行结果持续改进被测程序。整个过程只需要仓库文件和 Bash，不依赖浏览器或额外的 Skill。
...
<Steps>
  ...
  <Step title="按问题读取证据">
    ...
    | 诊断目标 | 入口 | 输出必须包含 |
    | --- | --- | --- |
    | 查看一次 Attempt 摘要 | `niceeval show @<id>` | Eval 断言、执行步骤、生命周期阶段耗时摘要、diff 摘要及各块可用性 |
    | 检查 Eval 断言及其通过或失败原因 | `niceeval show @<id> --eval` | 运行时 Eval 源码、源码哈希、断言所在行、严重度、分数与原因 |
...
`show` 的默认结果可能合成自多次运行：每个 Experiment × Eval 选择最新判定，因此局部重跑后仍能看到其它 Eval 的旧结果。该视图用于查看每条 Eval 的最新已知判定；同一版代码的整体结果必须通过一次 `--force` 全量运行确认。
