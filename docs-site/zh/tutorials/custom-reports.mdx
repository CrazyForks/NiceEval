...
命令行的范围先作用在挑选上，报告拿到的就是收窄到这个范围后的 `selection`：位置参数的评估用例 id 前缀收窄 Selection 覆盖的评估用例（覆盖提醒的分母同样收窄到范围内），`--results` 把结果根换成指定目录，`--exp` 让 Selection 只留该实验。`--history` 与 `--report` 互斥——趋势在报告里用 `exp.snapshots` 自行组织；证据视图（`--eval` / `--execution` / `--diff`）只看证据，不渲染报告。
