...
- 不同 sandbox provider。
- 不同运行环境条件（比如装不装某个记忆工具的二进制、有没有预置状态）：环境差异写在 `sandbox` spec 的 `.setup()` / `.teardown()` 钩子里，一个变体一个 experiment 文件，见 [Sandbox provider · 环境钩子](/zh/tutorials/sandbox-providers#环境钩子)。
- 同一任务的 pass@N。
