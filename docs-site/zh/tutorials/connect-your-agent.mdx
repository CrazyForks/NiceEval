...
description: "写一个 Adapter、配置一个 Experiment，并运行第一条 Eval；再把配置和参数从 Experiment 传给 Adapter 与被测应用。"
---

把一个被测对象接入 [NiceEval](https://niceeval.com/) 需要一个 **Adapter**。`defineAgent` 包住一个 `send` 函数；该函数接收输入、驱动 Agent，并返回本轮结果。

本教程先用 Adapter、Experiment 和 Eval 跑通最小接入，再说明参数从 Experiment 到 Adapter 和被测应用的传递路径。事件流、多轮、HITL 与 tracing 属于可选能力，文末列出对应教程。

## 按被测对象选择接入方式

<CardGroup cols={3}>
  <Card title="AI SDK 应用" icon="bolt" href="/zh/reference/builtin-agents">
    Vercel AI SDK（`useChat` 后端）应用可使用内置 `uiMessageStreamAgent` 连接现有 HTTP 接口，无需手写事件映射，并支持 HITL。直接按完整的 before/after 示例接入。
  </Card>
  <Card title="Coding agent CLI" icon="terminal" href="/zh/tutorials/sandbox-agent">
    评 claude-code / codex / bub 这类改文件的 coding agent：用内置 sandbox agent。
  </Card>
  <Card title="其它 AI Agent" icon="plug">
    自研 Agent loop、LangGraph / OpenAI Agents SDK 应用和已部署 Agent 需要手写 `send`（本文与[编写 Send](/zh/tutorials/write-send)）。应用已有 OTel 埋点时，再按 [OTel 接入](/zh/tutorials/connect-otel)发送 span，`niceeval view` 会显示调用瀑布图。
  </Card>
</CardGroup>

## 跑通最小接入

以下步骤假设项目已经运行 `npx niceeval init`，并包含 `niceeval.config.ts` 和 `evals/` 目录。三个文件各有一项职责：**Adapter 连接被测系统，Experiment 固定运行配置，Eval 定义交互和断言。**
...
最小示例先使用固定 URL。需要按环境传入 URL 时，使用下文的[参数传递方式](#传递参数：experiment-声明，adapter-消费)。`send` 的完整契约（`TurnInput` / `AgentContext` / `Turn` 各字段）见 [Adapter](/zh/explanation/adapter)。

即使 Agent runtime 和 Eval 位于同一个代码库，也应通过 HTTP 调用，不要把 `fetch` 换成进程内函数调用：

- **直调评的不是用户走的那条链路。** HTTP 层、序列化、中间件、流式传输全被绕过，eval 通过不代表线上行为正确。
- **进程不隔离，结果不可复现。** 被测代码的崩溃、全局状态、内存泄漏会连累运行器；并发跑多条 eval 时共享状态互相污染。
- **超时与取消会失效。** `ctx.signal` 只有挂在请求上才能取消超时的 Agent 轮次；进程内函数调用无法由该信号终止。
- **Adapter 无法复用于其它部署环境。** HTTP Adapter 只需更换 `baseUrl`，即可连接本地、预发或生产环境（见下文两个 Experiment 文件）；进程内调用依赖当前代码库。

因此，本地评估也需要先启动应用的开发服务器。
...
**验证运行结果**：终端会显示动态 dashboard，完成和排队数量在原位更新；失败、错误和 warning 会保留在输出中。运行结束后会打印摘要、失败 locator 和结果路径。`npx niceeval view` 会显示每条 Eval 逐轮的输入、事件和评分明细。
...
把第一步的 `my-bot` 从固定 URL 的实例改成接收配置的工厂。将 default export 改为返回 `defineAgent(...)` 的函数，并让 `send` 读取工厂参数。Experiment 声明的 `model` 和 `flags` 每轮经 `ctx` 到达，再由 `send` 随请求转发：
...
不要把 URL 放进 CLI 位置参数——experiment 名之后的位置参数只用于过滤 eval ID。experiment 的完整字段（`runs`、`budget`、并发、`sandbox`）见[写实验](/zh/tutorials/write-experiment)。

## 添加可选能力

最小接入完成后，可以按需扩展 Adapter。已有 Eval 不需要修改：
