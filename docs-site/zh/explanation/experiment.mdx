...
Experiment 还有一个实验级的 `setup` 钩子：整个实验只跑一次，在你自己的机器上执行，用来启动所有 Attempt 共享的服务（比如一条到内网服务的隧道、一个实验专用的 mock server）；`setup` 返回的清理函数会在这个实验全部 Attempt 结束后执行。要按实验在沙箱里准备环境（装二进制、预热、跨 attempt 存取状态），则挂在 `sandbox` 字段的 spec 上——`dockerSandbox()` 等工厂返回的对象可以链 `.setup()` / `.teardown()`。两者的边界与写法见[写实验 · 启动 Experiment 共享服务](/zh/tutorials/write-experiment#启动-experiment-共享服务)与[沙箱 provider · 环境钩子](/zh/tutorials/sandbox-providers#环境钩子)。

同一实验里的 eval 需要不同预制环境时，eval 只声明 provider-neutral 的 `environment` profile；sandbox spec 的 `environments` 表再把 profile 映射到 Docker image、E2B template 或 Vercel sandbox snapshot。这样任务需求留在 eval，provider 产物留在 spec，一个实验仍覆盖全部 eval、对比不拆分。写法见[写实验 · 让不同 eval 使用不同预制环境](/zh/tutorials/write-experiment#让不同-eval-使用不同预制环境)。
