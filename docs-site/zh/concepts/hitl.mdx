...
- **回答轮不是新对话。**它发生在同一条会话线上，`ctx.session.state` 还是同一份——Adapter 靠它找回上一轮挂起的现场。
...
"挂起的现场"（比如读了一半的 SSE 流）存在本会话线的 `ctx.session.state` 里，官方件 `pausable()` 替你存取——停轮时 `hold`，回答轮 `take`。怎么和流式驱动拼起来，见[写 send](/zh/guides/write-send#第五步：hitl) 第五步的完整骨架。

和 NiceEval 的其它能力一样，HITL 没有布尔声明：**做到了就是有**。send 返回过 `"waiting"` 并吐了 `input.requested`，`t.respond` 就能工作；没做到，eval 会在第一个 `parked()` 或 `requireInputRequest()` 上明确失败，而不是静默假通过。能力如何从构造中来，见 [Adapter](/zh/concepts/adapter#能力从哪来：构造证明，不是问卷)。
...
- [写 send](/zh/guides/write-send) — Adapter 侧实操：`pausable()` 件与流式 + HITL 的完整骨架。
