...
再进一步之前，把接入的两个档次说清楚。按「要不要改你应用的内部代码」，接入分两档，上面全部步骤都是 **Tier 1（无侵入）**：应用代码一行不改，adapter 只适配应用现有对外接口，观测数据走 OTel 或接口返回里已有的信息。这一档买到的是观测类断言（工具调用、trace 瀑布、用量/成本）和**模型对比** experiment——前提是应用接口本身暴露模型选择，experiment 的 `model` 会经 `ctx.model` 传给你的 adapter。

要评「改 prompt / 换工具集 / 开关某个 feature 谁更好」这类 **feature A/B test** 时，对照点在应用内部，无侵入拿不到——这是 **Tier 2（侵入）**：变更内部代码，把变体暴露成外部可选的配置（经 experiment 的 `flags` → `ctx.flags` 透传），adapter 仍然只经对外接口调用应用。
