...
完整的可跑参考：五个[无侵入接入示例](/zh/example/tier1-ai-sdk-v7)，覆盖事件流断言、多轮隔离、HITL 批准 / 拒绝、OTel 派生、trace。手写 `send` 时真正要写的只有 transport 和映射表——会话续接、HITL 暂停恢复的状态槽都在 `ctx.session` 上，逐帧驱动是官方件，见[内置 Agent 能力](/zh/reference/builtin-agents)。
