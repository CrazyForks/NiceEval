...
✗ @2h8m4k1 fixtures/button [compare/claude-e2b] errored · sandbox provision
...
niceeval show @1c3h6twx --timing    # 该 Attempt 的生命周期阶段耗时:排队、Sandbox 启动、setup 每个钩子、测试、评分、收尾
...
Sandbox 创建、setup 或 teardown 错误不依赖 trace。`result.json` 保存错误的 `code`、`message`、lifecycle operation、有限 cause/stack 和 diagnostics；trace 只在存在时补调用关系与耗时。Attempt 在 cleanup、teardown 和 sandbox stop 结束后才封口写入，因此收尾 diagnostic 也能回顾。
...
`--timing` 回答「时间花在环境还是 agent、卡在哪个生命周期阶段」。每次 Attempt 运行时，runner 都把各阶段的耗时记进 `result.json`：排队等并发额度、Sandbox 启动、setup 链上的每个钩子、agent 安装、测试主体、评分，以及收尾（teardown 与 Sandbox 销毁）。`--timing` 把这份数据排成一张表，`sandbox.setup` 展开到链上每个钩子；出错或超时的 Attempt 在最后一行用 `✗` 直接标出死在哪个阶段：
...
baseline             0.2s
agent.setup         41.5s
test                50.9s ✗ failed here (agent-runtime-error)
...
收尾阶段不计入 Attempt 总耗时，单独分组列出——「判定早就出了、进程还在等收尾」这类问题看这一组。它和 `--execution` 分工明确：`--timing` 看 NiceEval 侧的环境与调度耗时，不依赖 OTel；`--execution` 看 agent 内部每一步做了什么、花了多久。一次超时到底是 agent 干活慢，还是 Sandbox 启动和安装吃掉了预算，`--timing` 一眼可判。旧结果或第三方写入的结果没有阶段数据时，输出如实提示 `phase timing unavailable`。
...
`--eval` 回答“这道题实际检查了什么、哪条断言为什么过或没过”，`--execution` 回答“agent 做了什么、调用了什么、时间花在哪里”，`--timing` 回答“时间花在环境还是 agent、卡在哪个生命周期阶段”，`--diff` 回答“沙箱里的文件实际改成了什么”。先按问题选证据，不需要每次把四份都读完。
...
网页版多几样浏览操作：点表头就地排序、在榜单的过滤框里筛行、点开一个 experiment 行看它每道题的判定与原因、悬停散点看数值——这些只影响眼前的视图，不改判定口径，刷新即恢复。Attempt 弹窗里有阶段耗时分解，与 `show --timing` 是同一份数据：Sandbox 启动、setup 每个钩子、agent 安装、测试、评分与收尾各占多少，图形化一眼可读。报告上方有 **Copy fix prompt** 按钮，把全部失败打包成可直接粘给 coding agent 的修复 prompt（attempt 弹窗里有单条版）。报告文案有中英两份，随界面语言切换。
...
OTLP trace 或标准化后的 span 数据。它是可选的执行关系和耗时证据，不是错误存储：Sandbox provision 可能早于 telemetry，teardown 可能晚于 trace collect。
...
- Sandbox eval 跑得慢或超时，先看 `--timing`：排队、Sandbox 启动、setup 每个钩子、agent 安装、测试主体各花多久，超时到底是 agent 干活慢还是环境准备吃掉了预算，一眼可判；收尾卡住也在这里单独列出。
