...
- **装 Skill**：`skills: ["org/repo"]`（GitHub 仓库），`setup` 阶段跑 `npx skills add <org/repo>`，结果写进沙箱工作区的 `skills-lock.json`，claude CLI 启动时自动读取。
...
- **装 Skill**：`skills: ["org/repo"]`，和 claude-code 一样走 `npx skills add`。
