...
Claude Code 使用 `correctroads-default-team/niceeval-claude-code:v0.6.1`，Bub 使用 `correctroads-default-team/niceeval-bub:v0.6.1`。版本 tag 适合 CI；省略 tag 会跟随当前稳定构建。如何继续增加系统包、二进制或模型缓存，见 [Sandbox 后端 · 从官方基线继续构建以提速](/zh/guides/sandbox-providers#从官方基线继续构建以提速)。
...
运行 `niceeval show @2h8m4k1` 可查看结构化错误、diagnostics 和已完成的生命周期阶段；`--timing` 给各阶段的耗时分解（排队、Sandbox 启动、setup 每个钩子、测试、收尾），可直接看出错误或超时发生在哪一步、之前的时间花在哪里；有 OTel trace 时再用 `--execution` 查看执行树。Sandbox 创建失败可能发生在 telemetry 建立前，所以错误回顾不依赖 trace。
