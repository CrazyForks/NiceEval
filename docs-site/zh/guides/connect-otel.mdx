...
这是 **Tier 1（无侵入）接入**的观测手段：应用代码一行不改，观测和模型对比 experiment 都能做（见[接入分两个 Tier](/zh/guides/connect-your-agent#接入分两个-tier)）。要对照应用内部的功能变体（feature A/B test）则属于 Tier 2，OTel 替代不了。
