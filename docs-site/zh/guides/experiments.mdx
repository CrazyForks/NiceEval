...
sidebarTitle: "实验矩阵"
description: "使用 NiceEval experiments 让同一批 eval 横跨多个 agents、models 和 params，比较 pass rate、成本和延迟。"
---
...
`defineExperiment` 的字段怎么写、`params` 怎么透传到 adapter，见[写实验](/zh/guides/write-experiment)。

## 适合比较什么

- 不同 Adapters。
- 不同模型（Tier 1 接入即可：应用接口暴露模型选择，`model` 经 `ctx.model` 透传）。
- 不同 prompts 或 feature 开关（要求 Tier 3 接入：变体在应用内部，需要应用把它暴露成 experiment 可选的配置，经 `params` → `ctx.params` 透传）。
- 不同 sandbox 后端。
- 同一任务的 pass@N。
