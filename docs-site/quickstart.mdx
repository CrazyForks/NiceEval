...
Connecting your own application takes three files: an **adapter** (how to call your app), an **experiment** (who to evaluate, how many runs), and an **eval** (what to assert). Below is the shortest complete path. Scenario-specific guides and the coding-agent handoff path come after it.
...
<Steps>
  ...
  <Step title="View results">
    ```bash
    pnpm exec niceeval show         # terminal summary, easy to hand straight to an AI
...
The adapter is the layer between your agent and [NiceEval](https://niceeval.com/). There are three integration tiers, from non-invasive to partially instrumented, which unlock more useful eval features. See [Tier](/concepts/tier).
