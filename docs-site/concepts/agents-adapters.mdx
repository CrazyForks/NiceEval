...
description: "Learn how niceeval connects to any AI through named agent adapters. Direct agents wrap your own runtime; sandbox agents run CLI tools in isolation."
---
...
- **Adapter** is the concrete implementation. You write it (or use one of niceeval's built-ins). It knows how to authenticate, how to call your runtime, and how to translate whatever your AI returns into the standard event stream.
...
Regardless of whether the adapter drives your own runtime or a sandbox CLI, every agent exposes the same interface to the runner:
...
### `defineAgent` — direct agents

Use `defineAgent` when your subject under test is an agent you can drive directly. The `send` method is entirely in your control: call app code, fire a `fetch`, or do anything else. Your only obligation is to map the response to `StreamEvent[]`.
...
`toStreamEvents` is a small mapping function you write once — it converts "what your agent said and which tools it called" into `StreamEvent[]`. That is the entirety of a direct agent author's work.
...
<CardGroup cols={2}>
  <Card title="Direct agent" icon="plug">
    Implement `defineAgent` with a `send` that calls your runtime, maps the response to `StreamEvent[]`, and declares the capabilities your agent supports. That is everything.
  </Card>
...
- [Drive](/concepts/drive) — `t.send()`, `t.newSession()`, and HITL: how capabilities shape what you can do with a `Turn`.
- [Assert](/concepts/assert) — the full assertion vocabulary that runs against the standard event stream.
