...
An experiment is a `defineExperiment` configuration that describes one agent, one model, how many times to run each eval, and which evals to include. When you run `npx fasteval exp <group>`, the runner executes each config in that group under the same bounded-concurrency scheduler and produces a comparative report.
